...
void TIM3_IRQHandler()
{
  if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)
  {
		...
		Rt= (RegularConvData[5] *2050 )/ RegularConvData[6];
		...
		Ut = (RegularConvData[2] * CalibrationData.CalibrationValueForVoltage1) / RegularConvData[6];
		U_OUTtmp = MedianFilter(Ut);
		U_OUT_ForSetResistance = U_OUTtmp;
		if (U_OUTtmp<3) U_OUTtmp = 0;
		Ut = (RegularConvData[4] * CalibrationData.CalibrationValueForVoltage2) / RegularConvData[6];
		U_IN = middle_of_3Umax(Ut);
		Ut = (RegularConvData[1] * CalibrationData.CalibrationValueForCurrent*10) / RegularConvData[6] ;//  Current A/10
		Current_Out= MedianFilter1(Ut);

		Ut= (RegularConvData[0] * CalibrationData.CalibrationValueForCurrent1*10) / RegularConvData[6] ;//  Current A/10
